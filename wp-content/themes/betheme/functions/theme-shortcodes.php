...
if( ! function_exists( 'sc_shop_slider' ) )
{
	function sc_shop_slider( $attr, $content = null )
	{
		...
		$output = '<div class="shop_slider" data-count="'. $query_shop->post_count .'">';
...
// 		if( $width ) $class_div .= ' inline-block';
		...
		$image_output = '<img class="scale-with-grid" src="'. $src .'" alt="'. $alt .'" '.$width.' '.$height.' />';
...
if( ! function_exists( 'sc_hover_color' ) )
{
	function sc_hover_color( $attr, $content = null )
	{
		...
		$output = '<div class="hover_color align_'. esc_attr( $align ) .'" style="background:'. $background_hover .';border-color:'. $border_hover .';'. esc_attr( $style ) .'" ontouchstart="this.classList.toggle(\'hover\');">';
